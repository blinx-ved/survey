...
const q4_1 = __importDefault(require("./component/q4"));
...
function initializeQuestionObjects(questions) {
    return questions.map((question) => (Object.assign(Object.assign({}, question), { id: self.crypto.randomUUID(), value: null })));
}
function ProqolForm() {
    const [stressorQuestionObjects, setStressorQuestionObjects] = (0, hooks_1.useState)(initializeQuestionObjects(stressorQuestions));
    const [demoQuestionObjects, setDemoQuestionObjects] = (0, hooks_1.useState)(initializeQuestionObjects(demoQuestions));
    const [dassyQuestionObjects, setDassyQuestionObjects] = (0, hooks_1.useState)(initializeQuestionObjects(dassyQuestions));
    const [CBIpersonalQuestionObjects, setCBIpersonalQuestionObjects] = (0, hooks_1.useState)(initializeQuestionObjects(CBIQuestions));
    const [academicQuestionObjects, setAcademicQuestionObjects] = (0, hooks_1.useState)(initializeQuestionObjects(academicQuestions));
    const [passwordQuestionObjects, setPasswordQuestionObjects] = (0, hooks_1.useState)(initializeQuestionObjects(password));
    const handleChange = (0, hooks_1.useCallback)((id, newValue, questionObjects, setQuestionObjects) => {
        setQuestionObjects((prev) => prev.map((question) => (question.id === id ? Object.assign(Object.assign({}, question), { value: newValue }) : question)));
    }, []);
    ...
    return (<form className="w-full mx-auto md:max-w-xl px-2" onSubmit={handleSubmit}>
        <h1 className="text-2xl font-semibold mb-2 text-gray-80">Form</h1>
        <section className={styles_1.default.formSection}>
            <div className={styles_1.default.formHeader}>
                <p className={styles_1.default.formHeaderText}>Answer theses demographic to the best of your ability.</p>
            </div>
        <q4_1.default questions={demoQuestionObjects} handleChange={(id, newValue) => handleChange(id, newValue, demoQuestionObjects, setDemoQuestionObjects)}/>
        </section>
        <section className={styles_1.default.formSection}>
            <div className={styles_1.default.formHeader}>
                <h2 className={styles_1.default.formHeaderText}>Please evaluate the impact of each stressor on your well-being and daily life.</h2>
                <p className={styles_1.default.formHeaderSubText}>Select 0 if the stressor has no significant impact on your well-being or daily life.</p>
                <p className={styles_1.default.formHeaderSubText}>Select 1 if the stressor causes minor stress or concern but is manageable.</p>
                <p className={styles_1.default.formHeaderSubText}>Select 2 if the stressor is causing a moderate level of stress, noticeably affecting your well-being and daily life.</p>
                <p className={styles_1.default.formHeaderSubText}>Select 3 if the stressor is causing a high level of stress and concern, significantly impacting your well-being and making it difficult to cope.</p>
            </div>
            <q4_1.default questions={stressorQuestionObjects} handleChange={(id, newValue) => handleChange(id, newValue, stressorQuestionObjects, setStressorQuestionObjects)}/>
        </section>
        <section className={styles_1.default.formSection}>
            <div className={styles_1.default.formHeader}>
                <h2 className={styles_1.default.formHeaderText}>We would like to find out how you have been feeling in THE PAST WEEK. There are some sentences below. Please select the number which best shows how TRUE each sentence was of you during the past week. There are no right or wrong answers.</h2>
                <p className={styles_1.default.formHeaderSubText}>If the statement was NOT TRUE of you (in the past week), select 0.</p>
                <p className={styles_1.default.formHeaderSubText}>If the statement was A LITTLE TRUE of you, select 1.</p>
                <p className={styles_1.default.formHeaderSubText}>If the statement was FAIRLY TRUE of you, select 2.</p>
                <p className={styles_1.default.formHeaderSubText}>If the statement was VERY TRUE of you, select 3.</p>
            </div>
            <q4_1.default questions={dassyQuestionObjects} handleChange={(id, newValue) => handleChange(id, newValue, dassyQuestionObjects, setDassyQuestionObjects)}/>
        </section>
        <section className={styles_1.default.formSection}>
            <div className={styles_1.default.formHeader}>
                <p className={styles_1.default.formHeaderText}>Please answer these questions honestly based on your own experiences.</p>
            </div>
        <q4_1.default questions={CBIpersonalQuestionObjects} handleChange={(id, newValue) => handleChange(id, newValue, CBIpersonalQuestionObjects, setCBIpersonalQuestionObjects)}/>
        </section>
        <section className={styles_1.default.formSection}>
            <div className={styles_1.default.formHeader}>
            <p className={styles_1.default.formHeaderText}>Answer theses acedemic history questions to the best of your ability.</p>
            </div>
        <q4_1.default questions={academicQuestions} handleChange={(id, newValue) => handleChange(id, newValue, academicQuestionObjects, setAcademicQuestionObjects)}/>
        </section>
        <q4_1.default questions={passwordQuestionObjects} handleChange={(id, newValue) => handleChange(id, newValue, passwordQuestionObjects, setPasswordQuestionObjects)}/>
        <button className={styles_1.default.button} type="submit">Submit</button>
      </form>);
}
(0, preact_1.render)(<ProqolForm />, document.getElementById('app'));
